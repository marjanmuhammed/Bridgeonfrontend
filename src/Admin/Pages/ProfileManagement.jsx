...
const ProfileManagement = () => {
    ...
    try {
      ...
      for (const user of usersList) {
        try {
          ...
          profilesMap[user.id] = profileData;
        } catch (error) {
          console.log(`No profile found for user ${user.id}:`, error.message);
          profilesMap[user.id] = null;
        }
      }
      ...
      showToast("Failed to fetch users and profiles", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const hasProfile = (userId) => {
    return profiles[userId] !== null && profiles[userId] !== undefined;
  };
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200/60 p-6 mb-8">
          <div className="flex items-center justify-between">
            ...
            <div className="text-sm text-gray-500">
              {users.length} users • {Object.values(profiles).filter(p => p).length} profiles
            </div>
          </div>
        </div>
                      ...
                      <tr key={user.id} className="hover:bg-gray-50/50 transition-colors group">
                        ...
                        <td className="px-6 py-4">
                          <div className="text-sm text-gray-900 space-y-1">
                            ...
                            {!userProfile?.branch && !userProfile?.institution && (
                              <span className="text-gray-400">No academic info</span>
                            )}
                          </div>
                        </td>
